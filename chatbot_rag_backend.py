...
def chat_node(state: ChatState, config=None):
    ...
    doc_instruction = (
        f"A PDF is available. Use `rag_tool` with thread_id='{thread_id}' to answer document questions."
        if has_doc else
        "No PDF uploaded yet. Ask the user to upload one if they have document questions."
    )

    system_message = SystemMessage(content=(
        f"You are a helpful assistant with access to web search, stock prices, and a calculator.\n"
        f"{doc_instruction}"
    ))
